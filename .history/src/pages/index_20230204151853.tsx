...
const DeleteTodo = ({todo}: any) => {
 ...
 return {
   <div key={todo.id} className="grid grid-cols-2 gap-3">
                ...
                <button
                  ...
                  className="text-white-500 text-extra-bold w-1/3 bg-red-500 p-1"
                >
                  X
                </button>
              </div>
    ...
    <div className="w-300 h-200 grid min-h-full grid-cols-2 gap-5 text-white">
      <div className="w-90 h-200 min-h-max rounded bg-pink-500  p-3  ">
        ...
        {todos?.data?.map((todo) => {
          if (todo.status === "TODO") {
            return (
              <div key={todo.id} className="flex flex-row gap-3 md:grid-cols-3">
                <p className="w-2/3">{todo.name}</p>
                <button
                  name={todo.name}
                  onClick={(todo) => {
